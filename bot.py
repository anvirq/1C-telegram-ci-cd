...
@restrict()
async def regcas(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
async def updatedb(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if app_mode == 'prod':
        command = ['script.exe', infobase_name, db_user, db_pwd]
        cmd_description = "Обновление базы"
    else:
        command = ['oscript', 'script.os', infobase_name, db_user, db_pwd]
...
def main() -> None:
    ...
    app.add_handler(CommandHandler("regcas", regcas))
